...
export default function ABTestPage() {
  ...
  const [testType, setTestType] = useState<'email' | 'landing' | 'journey' | 'pricing'>('email');
      ...
      variants: [
        {
          ...
          allocation: 50,
          content: { subject: '【期間限定】外壁塗装が最大30%OFF！無料見積実施中' },
          results: {
            visitors: 2450,
            conversions: 147,
            conversionRate: 6.0,
        ...
        },
        {
          ...
          allocation: 50,
          content: { subject: '【今週末まで】外壁塗装の無料診断＆特別価格のご案内' },
          results: {
            visitors: 2380,
            conversions: 178,
            conversionRate: 7.5,
  ...
  const calculateStatisticalSignificance = (variantA: VariantResults, variantB: VariantResults) => {
    ...
    const significance = (1 - 2 * (1 - 0.5 * (1 + Math.erf(z / Math.sqrt(2))))) * 100;
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f5f5f5' }}>
                    ...
                    <CardContent>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          ...
                          <Chip
                            label={test.type === 'email' ? 'メール' : test.type === 'landing' ? 'LP' : 'ジャーニー'}
                            size="small"
                            color="primary"
                          />
                          <Chip
                        ...
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <IconButton size="small" onClick={() => setSelectedTest(test)}>
                            <Analytics />
                          </IconButton>
                      ...
                      <Grid container spacing={3}>
                        {test.variants.map((variant, index) => (
                          <Grid item xs={12} md={6} key={variant.id}>
                            <Paper sx={{ p: 2, bgcolor: test.winner === variant.id ? '#fff3e0' : '#f5f5f5' }}>
                              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                              ...
                              <Grid container spacing={2}>
                                <Grid item xs={6}>
                                  <Box>
                                    ...
                                    <Typography variant="h5" sx={{ fontWeight: 600, color: '#4285f4' }}>
                                      {variant.results.conversionRate}%
                                    </Typography>
                                    <Typography variant="caption" color="text.secondary">
                                      {variant.results.conversions} / {variant.results.visitors}
                                    </Typography>
                                  </Box>
                                </Grid>
                                <Grid item xs={6}>
                                  <Box>
                                    ...
                                    <Typography variant="h5" sx={{ fontWeight: 600, color: '#34a853' }}>
                                      ¥{(variant.results.revenue / 10000).toFixed(0)}万
                                    </Typography>
                                    <Typography variant="caption" color="text.secondary">
                                      単価: ¥{(variant.results.avgOrderValue / 10000).toFixed(0)}万
                                    </Typography>
                                  </Box>
                                </Grid>
                              </Grid>
                              ...
                              <Box sx={{ mt: 2 }}>
                                <LinearProgress
                                  variant="determinate"
                                  value={(variant.results.visitors / test.metrics.minimumSampleSize) * 100}
                                  sx={{ height: 8, borderRadius: 4 }}
                                />
                                <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5 }}>
                                  サンプル数: {variant.results.visitors} / {test.metrics.minimumSampleSize}
                                </Typography>
                              </Box>
                            </Paper>
                          </Grid>
                        ...
                        <Grid container alignItems="center" spacing={2}>
                          <Grid item xs={12} md={4}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Typography variant="h6" sx={{ fontWeight: 600 }}>
                                {test.confidence || calculateStatisticalSignificance(test.variants[0].results, test.variants[1].results).toFixed(1)}%
                              </Typography>
                              {(test.confidence || calculateStatisticalSignificance(test.variants[0].results, test.variants[1].results)) >= 95 ? (
                ...
                <TableBody>
                  {completedTests.map((test) => (
                    <TableRow key={test.id}>
                      ...
                      <TableCell>{test.startDate} ~ {test.endDate}</TableCell>
                      <TableCell>
                        <Chip
                    ...
                    <CardContent>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <Chip
                          ...
                          variant="outlined"
                        />
                        <Typography variant="body2" sx={{ color: '#34a853', fontWeight: 600 }}>
                ...
                <Paper sx={{ p: 3 }}>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={[
                      ...
                      { month: '8月', tests: 12, success: 9, improvement: 28 },
                      { month: '9月', tests: 10, success: 8, improvement: 35 },
                    ]}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line type="monotone" dataKey="tests" stroke="#4285f4" name="実施テスト数" />
                      <Line type="monotone" dataKey="success" stroke="#34a853" name="成功数" />
                      <Line type="monotone" dataKey="improvement" stroke="#fbbc04" name="改善率(%)" />
                    </LineChart>
                  </ResponsiveContainer>
                </Paper>
              </Grid>
      ...
      {/* 新規テスト作成ダイアログ */}
      <Dialog open={showNewTestDialog} onClose={() => setShowNewTestDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>
          <Typography variant="h6" sx={{ fontWeight: 600 }}>
            新規A/Bテスト作成
          </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setShowNewTestDialog(false)}>キャンセル</Button>
          <Button variant="contained" onClick={() => setShowNewTestDialog(false)}>
            テスト開始
          </Button>
        </DialogActions>
      </Dialog>
