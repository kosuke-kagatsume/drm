...
interface TestSegment {
  id: string;
  name: string;
  type: 'demographic' | 'behavioral' | 'geographic' | 'technographic' | 'construction';
...
export default function ABTestSettingsPage() {
  ...
  const [testType, setTestType] = useState<'ab' | 'multivariate' | 'split'>('ab');
  const [variants, setVariants] = useState<TestVariant[]>([
    { id: 'a', name: 'コントロール', allocation: 50, color: '#4285f4', content: {} },
    { id: 'b', name: 'バリアントB', allocation: 50, color: '#34a853', content: {} },
  ]);
  ...
  const segmentTemplates = [
    ...
    },
    {
      ...
      icon: <AttachMoney />,
      conditions: [
        { field: 'total_spent', operator: '>', value: 10000000 },
      ],
    },
    {
      name: '地方在住',
  ...
  const handleAddVariant = () => {
    const newVariant: TestVariant = {
      ...
      allocation: 0,
      color: ['#fbbc04', '#ea4335', '#9c27b0', '#00bcd4'][variants.length - 2] || '#666',
      content: {},
    };
    ...
    updatedVariants.push({ ...newVariant, allocation: 100 - (newAllocation * variants.length) });
  ...
  const handleRemoveVariant = (id: string) => {
    ...
    const updatedVariants = remainingVariants.map((v, index) => ({
      ...v,
      allocation: index === remainingVariants.length - 1
          ...
          <Grid item xs={12} lg={8}>
            <Paper sx={{ p: 3 }}>
              <Stepper activeStep={activeStep} orientation="vertical">
                ...
                <Step>
                  <StepLabel>
                    <Typography sx={{ fontWeight: 600 }}>バリアント設定</Typography>
                  </StepLabel>
                  <StepContent>
                    <Box sx={{ mt: 2 }}>
                      ...
                      {variants.map((variant, index) => (
                        <Paper key={variant.id} sx={{ p: 2, mb: 2, border: `2px solid ${variant.color}` }}>
                          <Grid container alignItems="center" spacing={2}>
                            ...
                            <Grid item xs={12} md={3}>
                              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                                <Chip
                                  label={index === 0 ? 'コントロール' : `テスト${index}`}
                                  size="small"
                                  sx={{ bgcolor: variant.color, color: 'white' }}
                                />
                                {variants.length > 2 && (
                                  <IconButton
                                    size="small"
                      ...
                      {totalAllocation !== 100 && (
                        <Alert severity="warning" sx={{ mb: 2 }}>
                          配分の合計が100%になるように調整してください（現在: {totalAllocation}%）
                        </Alert>
                      )}
                ...
                <Step>
                  <StepLabel>
                    <Typography sx={{ fontWeight: 600 }}>セグメント設定</Typography>
                  </StepLabel>
                  <StepContent>
                    <Box sx={{ mt: 2 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                    ...
                    <Box sx={{ mt: 2 }}>
                      ...
                      <Grid container spacing={2}>
                        {timeSlots.map((slot) => (
                          <Grid item xs={12} md={6} key={slot.day}>
                            <Paper sx={{ p: 2, opacity: slot.enabled ? 1 : 0.5 }}>
                                ...
                                <Switch
                                  ...
                                  onChange={(e) => {
                                    const updated = timeSlots.map(s =>
                                      s.day === slot.day ? { ...s, enabled: e.target.checked } : s
                                    );
                              ...
                              {slot.enabled && (
                                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                  <TextField
                                    ...
                                    onChange={(e) => {
                                      const updated = timeSlots.map(s =>
                                        s.day === slot.day ? { ...s, startTime: e.target.value } : s
                                      );
                                      setTimeSlots(updated);
                                    }}
                                  />
                                  ...
                                  <TextField
                                    ...
                                    onChange={(e) => {
                                      const updated = timeSlots.map(s =>
                                        s.day === slot.day ? { ...s, endTime: e.target.value } : s
                                      );
                                      setTimeSlots(updated);
                                    }}
                                  />
                      ...
                      <RadioGroup defaultValue="normal">
                        <FormControlLabel
                          ...
                          label={
                            <Box>
                              <Typography variant="body1">即座に配信</Typography>
                              <Typography variant="caption" color="text.secondary">
                                すべての対象者に一度に配信
                              </Typography>
                            </Box>
                      ...
                      <Paper sx={{ p: 2, mb: 2, bgcolor: '#f5f5f5' }}>
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={12} md={6}>
                            ...
                            <Typography variant="body1" sx={{ fontWeight: 600 }}>
                              {testType === 'ab' ? 'A/Bテスト' : testType === 'multivariate' ? '多変量テスト' : 'スプリットURL'}
                            </Typography>
                          </Grid>
                          <Grid item xs={12} md={6}>
                            <Typography variant="caption" color="text.secondary">
                          ...
                          <Grid item xs={12} md={6}>
                            ...
                            <Typography variant="body1" sx={{ fontWeight: 600 }}>
                              {segments.filter(s => s.enabled).reduce((sum, s) => sum + s.size, 0).toLocaleString()}人
                            </Typography>
                          </Grid>
                        </Grid>
                      </Paper>
